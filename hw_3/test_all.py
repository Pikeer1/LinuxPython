...
class TestPositiv:
    ...
    def test_step2(self, clear_folders, make_files):
        res = []
        res.append(self.checkout(f"cd {data['folder_tst']}; 7z a {data['folder_out']}/арх2",
                                 "Everything is Ok"))
        res.append(self.checkout(f"cd {data['folder_out']}; 7z e арх2.7z -o{data['folder_ext']} -y",
                                 "Everything is Ok"))
    ...
    def test_step6(self, clear_folders, make_files):
        ...
        for item in make_files:
            res.append(
                self.checkout(f"cd {data['folder_out']}; 7z l арх2.7z {data['folder_ext']}", f"{data['file_in_ext']}"))
    ...
    def test_step7(self, clear_folders, make_files, make_subfolder):
        ...
        res.append(self.checkout(f"cd {data['folder_tst']}; 7z a {data['folder_out']}/арх2",
                                 "Everything is Ok"))
        res.append(self.checkout(f"cd {data['folder_out']}; 7z x арх.7z -o{data['folder_ext']}",
                                 "Everithing is ok"))
        ...
        res.append(
            self.checkout(f"ls {data['folder_ext_2']} /", make_subfolder[0]))
